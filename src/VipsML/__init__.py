...
class PreCropModulationGenerator():
    ...
    def fetch(self, x, y, s_x, s_y, n):
        """
        Fetches a numpy array containing an image window
        with given coordinates and sizes with given modulation.

        Modulation is either:
            unchanged (0), 
            rotated stepwise 90 degrees (1-3), 
            flipped, and possibly rotated stepwise 90 degrees (4-7)

        :param x: x-coordinate
        :param y: y-coordinate
        :param s_x: with of window in pixels
        :param s_y: height of window in pixels
        :param n: modulation type
        """
...
class VipsScanImage():
    ...
    def __init__(self, vips_image, frame_size, 
                 ...
                 modulator_class=RegionModulationGenerator):
        """
        :param vips_image: Vips image to scan
        :param frame_size: size (x AND y) of frame to slide across image
        :param padding: How much of surrounding pixels to include, either single
            value or tuple
        :param outer_pad: Vips-constant for padding fill value for unknonwn pixels
        :param modulator_class: Class to use for modulation.

        The scanner will provide images of size frame_size + padding.
        """
        ...
        self.generate_modulation=modulator_class(self.padded).fetch
    ...
    def get(self, index, mod_id=0):
        """
        Returns frame X scanned from right to left, top to bottom.
        ...
        :param index: Frame no. in order from right to left, top to bottom.
        :param mod_id: modulation type
        """
...
class VipsML():
    """
    Wraps a Vips-image, optionally with a Vips mask for training, using given parameters.
    """
    def __init__(self, image, mask=None, frame_size=None, 
                 padding=None, outer_pad=['white', 'black'], meta_ratio=1):
        """
        Returns a tile generator with an optional mask embedded.

        :param image: Vips image or path to an image
        :param mask: Vips mask or path to mask. The mask can either be
            multi dimensionally binary or categorical in one channel.
        :param frame_size: size (x AND y) of frame to slide across image
        :param padding: How much of surrounding pixels to include, either single
            value or tuple
        :param outer_pad: Vips-constant for padding fill value for unknonwn pixels
        :param meta_ratio: If not 1, will include a scaled version of surrounding pixels
            to introduce metalevel information about tissue structure to the model
        """
    ...
    def prep_mask(self, mask, frame_size, padding, outer_pad):
        """
        Ensures the mask to be in compatible format with keras classification/segmentation systems
        """
        if mask == None:
            return None
        else:
    ...
    def get_training_pair(self, index):
        """
        Returns a pair of image + mask at given index, with random modulation.
        """
    ...
    def get_single(self, index):
        """
        Returns an image at given index.
        """
    ...
    def predict_model(self, model, batch_size=25):
        """
        Applies a keras predictor on an image.
...
class VipsSegmentationML(VipsML):
    """
    Wrapper for applying segmentation models, e.g. SegNet.
    """
...
class VipsClassificationML(VipsML):
    """
    Wrapper for applying classification models, e.g. VGGNet.
    The resulting classifier scales the image down by a factor of frame_size.
    """
...
class VipsGroupML(Sequence):
    """
    Keras sequence generator for tiled images and masks.
    """
    def __init__(self, images, batch_size=15):
        """ Returns a data generator for keras model training.
        :param images Pre-instantiated VipsML-images, optionally with masks.
        :param batch_size: Batch size for prediction/training.
        """
        self.images = images
    ...
    def split_generators(self, validation_ratio):
        """
        Returns a split set of generators, one for training, one for validation.
        :param validation_ratio: relative size of the validation dataset.
        """
        class SplitSet(Sequence):
            def __init__(self, image, start, end, shuffle=True):
                self.image = image
                self.start = start
                self.end = end
        ...
        return SplitSet(self, 0, split_point), SplitSet(self, split_point, len(self), shuffle=False)
    
    def __getitem__(self, index):
        """
        Returns a (index-shuffled) batch of image frames from the supplied datasets.
        """
    ...
    def get_training_pair(self, index):
        """
        Get a batch of image/mask pairs at (shuffled) index.
        """
        mod_n = random.choice(range(8))
